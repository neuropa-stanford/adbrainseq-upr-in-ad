...
ORDER = sorted(NAMES, key=lambda g: (float(idx[(g, "Thapsigargin", "full ranked list")]["K"] or 0) < 10,
                                     best.get(g, 2.0)))
ORDER = ORDER[:14]   # panel a: show terms 1-14 only (peripheral/GO-obsolete terms 15-18 omitted from display)
...
for ext in ("png", "svg"):
    fig.savefig(os.path.join(OUT, f"R1Q1_UPR_GSEA_14terms.{ext}"),
                dpi=300 if ext == "png" else None, facecolor="white", bbox_inches="tight")
print("saved R1Q1_UPR_GSEA_14terms.{png,svg}")
