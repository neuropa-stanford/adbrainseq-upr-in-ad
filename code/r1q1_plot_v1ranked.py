...
# --- re-order rows by rank: strongest adjusted P achieved in ANY condition
best = {}
for r in rows:
    if r["adjP"] and r["significant"] == "True":
        p = float(r["adjP"])
        best[r["GO"]] = min(best.get(r["GO"], 1.0), p if p > 0 else 1e-330)
ORDER = sorted(ORDER, key=lambda t: (t[0] in OBSOLETE, best.get(t[0], 2.0)))
RANK = {g: i + 1 for i, (g, _) in enumerate(ORDER)}
BEST = best
...
gs = fig.add_gridspec(2, 1, height_ratios=[3.1, 1.15], hspace=0.20,
                      left=0.315, right=0.965, top=0.905, bottom=0.115)
...
ax.set_yticklabels([f"{RANK[g]}.  {n}  ({g})" for g, n in ORDER], fontsize=9)
for tick, (g, _) in zip(ax.get_yticklabels(), ORDER):
    tick.set_color("#111" if g in CORE else "#8a8a8a")
    tick.set_fontweight("bold" if g in CORE else "normal")
...
ax.set_title("A   UPR-related GO terms RANKED by adjusted P value (rank 1 = most significant term overall)"
...
fig.text(0.335, 0.014,
         ...
         "UP/DOWN = genes changed in that direction; BOTH = union (direction-agnostic) and is therefore dominated by query size — interpret directional columns, not BOTH.\n"
         "Rows are ranked by the strongest adjusted P each term reaches in any condition; bold black labels = UPR/ERAD-core terms, grey = generic folding/trafficking parent terms.",
